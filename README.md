...
### Before following below steps, ensure you are in the root directory: dsan5400-Final-PerfumeGenerator 
...
Before we use website, we need to fine tune GPT2 model first, follow these steps:

Preprocess the data:
```python
python perfume_recommender/perfume_recommender/text_generation/precess_reviews.py
```

Fine-tune GPT-2:
```python
python perfume_recommender/perfume_recommender/text_generation/fine_tune_gpt2.py
```
The fine-tuned model will be saved to the root directory dsan5400-Final-PerfumeGenerator
...
├── extractreview.py  # extract perfume reviews from website
├── data_download.py  # download data from google drive
└── pyproject.toml
